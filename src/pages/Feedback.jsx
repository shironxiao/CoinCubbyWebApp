...
import { submitFeedback, fetchUserFeedback, hasFeedbackForTransaction } from '../lib/supabase'
...
export default function FeedbackPage({ session, rentalHistory, loadingData, t, lang }) {
  ...
  useEffect(() => {
    ...
    async function loadFeedback() {
      setLoadingFb(true)
      try {
        // Fetch all feedback the user has already submitted
        ...
        // Only completed rentals that don't have feedback yet are actionable
        ...
        // Initialise row state for pending items
        setRowState((prev) => {
          const next = { ...prev }
  ...
  async function handleSubmit(item) {
    ...
    try {
      ...
      // Move row from pending to submitted list in UI
      setSubmitted((prev) => [
        {
  ...
  const isLoading = loadingData || loadingFb
      ...
      {submitted.length > 0 && (
        <section className="fb-section">
          ...
          <div className="fb-cards">
            {submitted.map((fb) => {
              const lockerNum =
                fb.transactions?.lockers?.locker_number || '?'
              return (
