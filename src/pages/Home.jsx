      ...
      {activeRentals.length > 0 && (
        <div className="home-rentals-container">
          {activeRentals.map((rental) => {
            ...
            const sizeInfo     = sizeFromType(rental.lockers?.size_type_id, ratesMap)
            // Prefer the DB rate from the rates join, then fall back to sizeFromType
            const ratePerHr    = rental.rates?.price_per_hour != null
              ? Number(rental.rates.price_per_hour)
              : sizeInfo.rate
